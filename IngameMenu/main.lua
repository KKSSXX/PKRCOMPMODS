...
local function collect_menu_rows(rows, mod_table)
if not (mod_table and mod_table.menu_rows) then return end
local ok, mrows = pcall(mod_table.menu_rows, options, save_options, apply_skips, game_id())
if ok and type(mrows) == "table" then
for _, r in ipairs(mrows) do rows[#rows + 1] = r end
end
end
local function build_qol()
local rows = {}
collect_menu_rows(rows, TitleSkip)
rows[#rows + 1] = {
label = "Speech Skip",
kind = "check",
...
local previous_keyreleased = love.keyreleased
love.keyreleased = function(key, scancode)
if settingsOpen then
  return
end
if previous_keyreleased then return previous_keyreleased(key, scancode) end
end
